...
export async function fetchPosts(
  ...
  if (params.isNewLoad) queryParams.set("isNewLoad", "true");
...
export async function fetchUserBlogData(userId: string) {
  try {
    const response = await fetch(`${API_URL}/posts/@${userId}`);
    if (!response.ok) {
...
export async function processUserBlogData(userId: string) {
  try {
    const data = await fetchUserBlogData(userId);
    return data;
  } catch (error) {
    throw error;
  }
